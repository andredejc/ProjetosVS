...
IF OBJECT_ID('tempdb..#InformacaoJobs','U') IS NOT NULL BEGIN DROP TABLE #InformacaoJobs END

CREATE TABLE #InformacaoJobs (
	Package					VARCHAR(MAX),
	JobName					VARCHAR(MAX),
	IsEnabled				VARCHAR(MAX),
	JobCreatedOn			VARCHAR(MAX),
	JobLastModifiedOn		VARCHAR(MAX),
	StepNo					VARCHAR(MAX),
	StepName				VARCHAR(MAX),
	JobOwner				VARCHAR(MAX),
	JobCategory				VARCHAR(MAX),
	JobDescription			VARCHAR(MAX),
	StepType				VARCHAR(MAX),
	RunAs					VARCHAR(MAX),
	[Database]				VARCHAR(MAX),
	ExecutableCommand		VARCHAR(MAX),
	OnSuccessAction			VARCHAR(MAX),
	RetryAttempts			VARCHAR(MAX),
	RetryInterval			VARCHAR(MAX),
	OnFailureAction			VARCHAR(MAX),
	IsScheduled				VARCHAR(MAX),
	JobScheduleName			VARCHAR(MAX),
	ScheduleType			VARCHAR(MAX),
	Occurrence				VARCHAR(MAX),
	Recurrence				VARCHAR(MAX),
	Frequency				VARCHAR(MAX),
	ScheduleUsageStartDate	VARCHAR(MAX),
	ScheduleUsageEndDate	VARCHAR(MAX),
	ScheduleCreatedOn		VARCHAR(MAX),
	ScheduleLastModifiedOn	VARCHAR(MAX),
	JobDeletionCriterion	VARCHAR(MAX)
)

DECLARE 
	@Objeto	VARCHAR(500) = 'CDFINC',
	@PacoteNome VARCHAR(500)

DECLARE cursorPacote CURSOR FOR 
SELECT SUBSTRING(PacoteNome, 0, CHARINDEX('"', PacoteNome)) AS PacoteNome
FROM [msdb].[dbo].[sysssispackages]
	CROSS APPLY (
		VALUES (
			SUBSTRING (CAST(CAST(packagedata AS VARBINARY(MAX)) AS VARCHAR(MAX)),PATINDEX('%DTS:ObjectName="%',CAST(CAST(packagedata AS VARBINARY(MAX)) AS VARCHAR(MAX))) + LEN('DTS:ObjectName="'),100)
		)
	) AS Alias( PacoteNome )
WHERE CAST(CAST(packagedata AS VARBINARY(MAX)) AS VARCHAR(MAX)) LIKE '%' + @Objeto + '%'

OPEN cursorPacote

FETCH NEXT FROM cursorPacote INTO @PacoteNome

WHILE @@FETCH_STATUS = 0
BEGIN
	
	INSERT #InformacaoJobs
	SELECT
		@PacoteNome AS [Package],
	...
	WHERE REPLACE(REPLACE(REPLACE([sJSTP].[command], CHAR(10) + CHAR(13), ' '), CHAR(13), ' '), CHAR(10), ' ') LIKE '%' + @PacoteNome + '%'		

	FETCH NEXT FROM cursorPacote INTO @PacoteNome

END
CLOSE cursorPacote
DEALLOCATE cursorPacote

SELECT DISTINCT * FROM #InformacaoJobs